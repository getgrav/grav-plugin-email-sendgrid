 ...
 * back on {@see \Grav\Plugin\EmailSendgrid\Provider\SendGridProvider} makes
...
final class OlderEmailPluginTest extends TestCase
{
    public function testTheProviderLoadsWhenTheEmailPluginHasNoInboundTypes(): void
    {
        ...
        if (!\is_string($root) || trim($root) === '') {
            $root = \dirname(__DIR__, 4) . '/grav-plugin-email';
        }
        $contract = rtrim($root, '/') . '/classes/Providers';
        $classes = \dirname(__DIR__, 3) . '/classes';

        $script = <<<'PHP'
<?php
[$_, $contract, $classes] = $argv;
spl_autoload_register(static function (string $class) use ($contract, $classes): void {
    foreach (['Grav\\Plugin\\Email\\Providers\\' => $contract, 'Grav\\Plugin\\EmailSendgrid\\' => $classes] as $prefix => $dir) {
...
$class = interface_exists('Grav\\Plugin\\Email\\Providers\\Inbound\\InboundCapable')
    ? Grav\Plugin\EmailSendgrid\Provider\SendGridInboundProvider::class
    : Grav\Plugin\EmailSendgrid\Provider\SendGridProvider::class;
$provider = new $class();
$registry = new Grav\Plugin\Email\Providers\ProviderRegistry();
$registry->add($provider);
echo json_encode([
    'class' => get_class($provider),
    'key' => $registry->byKey('sendgrid')?->key(),
    'label' => $provider->label(),
    'inbound' => interface_exists('Grav\\Plugin\\Email\\Providers\\Inbound\\InboundCapable'),
]);
PHP;

        $file = tempnam(sys_get_temp_dir(), 'sg-old');
        ...
        self::assertSame(
            '{"class":"Grav\\\\Plugin\\\\EmailSendgrid\\\\Provider\\\\SendGridProvider","key":"sendgrid","label":"SendGrid","inbound":false}',
            trim((string)$output)
        );
    }
}
